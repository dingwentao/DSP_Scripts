...
r = size(Results.Chirps, 1);
c = size(Results.Chirps, 2);
...
for ii = 1 : ceil(c/chunk_size)
    ...
    %     save_path(ii, :) = [save_dir file0(1:end-4) '_chunk' ii_str(ii, :)];
%     figName(ii, :) = [file0(1:15) '_' file0(39:42) '_' ii_str(ii, :)];
    D = 2*params.coh_avg_size; 
    if first(ii)>D && last(ii)<c-D
        temp = ColoradoProcessorF1(Results.Chirps(:, first(ii)-D:last(ii)+D), params);
        [echogram(:, first(ii):last(ii))] = temp(:,1+D:end-D);
    else
        [echogram(:, first(ii):last(ii))] = ColoradoProcessorF1(Results.Chirps(:, first(ii):last(ii)), params);
    end
end
...
% save full echogram
save_path_fig = [save_dir file0(1:end-4) '.fig']; % '_chunk' ii_str(ii, :)];
save_path_jpg = [save_dir file0(1:end-4) '.jpg']; % '_chunk' ii_str(ii, :)];
