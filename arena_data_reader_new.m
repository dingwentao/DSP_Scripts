function [trace_time, Results] = arena_data_reader_new (xml_fullpath, dat_fullpath)

%% Code last updated by Zhe Jiang on April 11
...
tmp_fullpath = strcat(dat_fullpath,'.tmp');
radar_id = fopen(tmp_fullpath , 'w');
if radar_id == -1
    error('ERROR: count not open radar.dat. Exiting.');
else
...
if radar_id == -1
    error('ERROR: could not open radar.dat. Exiting.');
else
